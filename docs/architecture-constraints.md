...
## 12. Galería de Ofrenda: alto de la figura sin depender de `aspect-ratio` (v4.20.0 — galería retirada en v4.21.0)

> **Estado:** la galería `.ofrenda__galeria`/`.ofrenda__figura` ya no existe (v4.21.0: las 3 fotos son miniaturas de Historia/Archivos/Ofrendas con el patrón trigger + lightbox compartido, que tiene un `<img>` en flujo y no sufre este fallo). Se conserva como **lección de WebKit** para cualquier contenedor futuro con solo hijos absolutos; el test guardia de la galería se sustituyó por "la sección Ofrenda no lleva galería de fotos".
