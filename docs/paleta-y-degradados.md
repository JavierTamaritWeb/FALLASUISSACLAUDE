...
- Desde v4.30.41 también el teaser de «Deportes» de la home y, desde v4.37.1, `deportes.html` entero (`.deportes__heading-texto`, `__delegados-titulo`, `__tablon-titulo`), el hero interior de las 29 páginas (`.header-inner::before` en `$gradiente-celeste`, `.heading-inner` en `$coral-texto` y `__span` en `$azul-titulo-seccion`), el título del monumento (`.falla__monumento-title`), Nuevos Falleros (`__heading-texto`, `__subtitulo`) y el título del tablón (`.tablon-titulo`).
...
| --- | --- | --- |
| `body::before`, `.header-inner::before` (solo en oscuro desde v4.37.1), `.quieres-mas::before`, `.blog-detail__article::before` (solo en oscuro desde v4.37.1) | `$gradiente-institucional` en `::before` con `opacity` | `opacity: 0` sobre color sólido (`docs/global-styles.md`) |
...
| `.accordion__content`, `.historia__prologo`, `.deportes` (en claro `$gradiente-celeste` desde v4.37.1) | `$gradiente-institucional` en el fondo | `$negro` / velo `::before` |
