...
```scss
.falla {
  background: url('../img/elementos-UXUI/fondo_traje.png'); // Imagen fija
  position: relative;
...
La sección `.falla` (página principal con información de la Falla) tiene un fondo con imagen de traje regional valenciano (`src/img/elementos-UXUI/fondo_traje.png`).

### Implementación actual

```scss
.falla {
  background: url('../img/elementos-UXUI/fondo_traje.png');
  background-size: cover;
...
| --- | --- | --- |
| `$frieze-img` | `url('../img/elementos-UXUI/cenefa-barroca-4.seo.webp')` | Cenefa barroca (v4.40.1); derivado WebP de 512 px que el build genera desde `cenefa-barroca-4.png` vía `src/data/image-variants.json`, recortando con `extract` los márgenes transparentes verticales para que el motivo llene la franja (v4.40.2) |
