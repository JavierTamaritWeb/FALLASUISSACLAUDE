...
- **Pre-render i18n VA en build (v4.6.23):** El pipeline pre-renderiza valenciano en `gulpfile.js → prerenderTranslations()`. Reglas:
  ...
  2. **No tocar `data/translations.json` esperando que el HTML cambie sin rebuild**: el HTML pre-renderizado se genera en build time. Tras editar el JSON: `npm run build`.
  3. **Para añadir un nuevo atributo i18n** (p. ej. `data-i18n-aria-describedby`): hay que actualizar a la vez `prerenderTranslations` en `gulpfile.js` (array `attrMappings`) Y el array de selectores en `js/lang.js → updateTranslations()`. Ambos deben procesar el mismo set, o el toggle runtime y el build se desincronizarán.
  ...
  6. **`data-i18n-dynamic`** (meteo): el pre-render lo skipea. NO añadir contenido pre-renderizado a estos elementos — los rellena `js/meteo.js` en runtime con datos en vivo.
