...
> 3. **Overrides del `.accordion` base**: el `.accordion__content` base de `_falla.scss` aplica un gradiente azul institucional + `.accordion__content-inner` con `display: flex; flex-direction: column; align-items: center`. Dentro de `.accordion--representantes` esos se neutralizan a fondo `v.$blanco-hueso` (claro) / `v.$negro` (oscuro) y `display: block` para que el grid funcione. NO modificar el `.accordion` base.
> 4. **Titular `<button>` accesible**: el patrón antiguo del proyecto usa `<div class="accordion__titular">` (sin Enter/Space nativo). Para esta subsección se usa `<button type="button" class="accordion__titular" aria-expanded="false" aria-controls="...">` con focus-visible coral + override en `_representantes.scss` que neutraliza `background/border/font` nativos del botón. Los acordeones antiguos siguen siendo `<div>` — no se han migrado.
> 5. **`src/js/acc.js` extendido retro-compatible**: ahora sincroniza `aria-expanded` cuando el titular tiene ese atributo. Los acordeones antiguos (sin `aria-expanded`) siguen funcionando exactamente igual. NO se toca el atributo `hidden` ni `inert` en el panel: la animación `max-height: 0 → 100rem` del `.accordion` base depende de que el `.accordion__content` quede en el flujo del DOM. Para a11y, el `aria-expanded` del botón es suficiente para que los lectores de pantalla anuncien estado expandido/colapsado.
> 6. **i18n** (sólo ES/VA — EN/FR no tienen bloque `archivos`): bloque `historia.archivos.representantes` con `titulo`, `edicion202425/202526`, `enConstruccion`, y `cargos.{falleraMayor,presidente,falleraMayorInfantil,presidenteInfantil}`. Pre-renderizado en VA en `dist/va/`.
> 7. **IDs únicos por página**: en `index.html` se usan IDs `representantes-2025-26-index` / `representantes-2024-25-index`; en `lafalla.html` los IDs llevan sufijo `-lafalla` para evitar cualquier colisión potencial al navegar con anchors o si en el futuro se inyecta uno en otro.
> 8. **Jerarquía tipográfica `h5 → h6`**: el `.accordion__header` base sube hasta `2.4rem` en desktop, lo que igualaba o superaba al `.historia__archivos-subtitulo` h5 (`1.8rem`). Dentro de `.accordion--representantes` se sobrescribe a `1.3rem` (mobile) / `1.4rem` (768-1024px) / `1.5rem` (≥1025px) — siempre claramente menor que el subtítulo en cada breakpoint, manteniendo coherencia con el orden semántico h5 → h6.
> 9. **Modo oscuro afinado**: la regla global `body.modo-oscuro .accordion__titular { background: v.$negro }` se sobrescribe con `body.modo-oscuro .accordion--representantes .accordion__titular { background: v.$negro; color: v.$blanco-hueso }` (especificidad 0,0,3,0) para neutralizar el coral residual del modo claro. `:hover` cambia a `v.$negro-casi` para que el feedback siga siendo visible. La `box-shadow` de la imagen pasa de `rgba(0,0,0,…)` (invisible sobre negro) a `rgba(255,111,97,.18-.32)` (glow coral suave). El nombre se vuelve coral en oscuro (consistente con `.accordion__nombre-plana-mayor` de la Plana Mayor) y los cargos/placeholder usan `v.$gris-muy-claro` sobre fondo `v.$negro` (contraste ≈14:1 — WCAG AAA).
>
