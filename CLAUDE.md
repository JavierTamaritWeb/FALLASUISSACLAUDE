...
- **Cookie Banner Safari fixes (v4.6.10):** The `cookie-banner.js` script includes two critical workarounds for macOS / iOS Safari:
  ...
  2. **Reflow & Transition Bug**: Safari frequently skips CSS transitions (`transform: translateY(100%) -> 0`) if the transition class is appended in the same frame as the banner's insertion into the DOM. The script MUST force a manual layout reflow with `void banner.offsetWidth;` directly before adding the `--visible` class. Do NOT replace this with `requestAnimationFrame`.
